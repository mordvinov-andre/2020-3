...
// Параметр feedrate принимает фидрейт всех четырех кареток. Пареметр s2 принимает отключение проверки byteval('S') для оси Z.

static void axisRoboMove(float A, float B, float C, float D, float feedrate = 50.0, bool s2 = true){

  // float feedrate = parser.linearval('F') > 0.0 ? MMM_TO_MMS(parser.value_feedrate()) : feedrate_mm_s 21.11.20 Миша
...
static void homeaxis(const AxisEnum axis) {//Родион заметка
  if (axis == Z_AXIS||axis == X_AXIS||axis == Y_AXIS){
...
void searchB(const AxisEnum axis, float MishaLox){
  LOOP_XYZE(i) {
    if (axis==i) {
      float v = MishaLox;//позволяет задавать перемещение на число, а не на координату
      destination[i] = (axis_relative_modes[i] || relative_mode)
        ? current_position[i] + v
        : (i == E_CART) ? v : LOGICAL_TO_NATIVE(v, i);
    }
  ...
  if (parser.linearval('F') > 0)
    feedrate_mm_s = MMM_TO_MMS(parser.value_feedrate());//не трогать скорость
...
inline void RoboSearch(const AxisEnum axis, float Poboshift) {
  if (IsRunning() && G0_G1_CONDITION) {
    searchB(axis, current_position[axis]+Poboshift);// For X Y Z E F
...
}//Кустом поиск Кирпича Родион 22.11.20
...
/* 11.11.2020 Андрей Р. Функция задержки, принимает два аргумента:
 *  dwell_ms - время задержки
 *  milli - флажок, если milli = false (по стандарту) - аргумент dwell_ms в секундах, иначе в миллисекундах.
 */

void RoboDelay(millis_t dwell_ms, bool milli = false) {

  if (!milli) dwell_ms *= 1000;
...
inline void gcode_G4() {
  ...
  inline void gcode_G7() {//Родион 08.11.20 колхозная Г28
    bool count_it = false;
    #if ENABLED(NO_MOTION_BEFORE_HOMING)
      if (axis_unhomed_error()) return;
    #endif
...
if (digitalRead(14)==HIGH||digitalRead(3)==HIGH){
...
if (IsRunning()) {
      ...
      planner.buffer_segment(go[A_AXIS], go[B_AXIS], go[C_AXIS]
        ...
        SERIAL_PROTOCOLPAIR(" A:", planner.get_axis_position_mm(A_AXIS));
        SERIAL_PROTOCOLPAIR(" B:", planner.get_axis_position_mm(B_AXIS));
        SERIAL_PROTOCOLPAIR(" C:", planner.get_axis_position_mm(C_AXIS));
        SERIAL_PROTOCOLPAIR(" D:", planner.get_axis_position_mm(D_AXIS));
        }

#endif

inline void Redge(const AxisEnum axis, float Roboshift){
  bool robotyck = (digitalRead(18) < 1); // Едем до упора по X.
  //bool roboZ = 0.0;
  float dX = 0.0;
  float CP;
RoboSearch(axis, -Roboshift);
  robotyck = (digitalRead(18) < 1);
  while (robotyck) {
    RoboSearch(Y_AXIS, -0.01);
    robotyck = (digitalRead(18) < 1);
    }

    // Откатываемся назад, поднимаемся, возвращаемся к детали, мерием Z.
    RoboSearch(Y_AXIS, 10.0); 
      
    RoboSearch(Z_AXIS, 10.0);

    
  RoboSearch(Y_AXIS, -10.5); 
  robotyck = (digitalRead(18) < 1);
  
  while (robotyck){
  RoboSearch(Z_AXIS, -0.01);
  robotyck = (digitalRead(18) < 1);
}
...
inline void TycK(const AxisEnum axis, float dist, int num){
  int ir;
  float rshift=dist/(num+1);
  for (ir=0;ir<num;ir++){
    Redge(axis, rshift);
  }
}

float Sdistance (const AxisEnum axis, int rdir){
  
  bool robotyck = (digitalRead(18) < 1);
  float Wdist=0.0;
  float KT1,KT2;
  KT1=0.0; KT2=0.0;
  
  while (robotyck){
    RoboSearch(axis,rdir*0.01);//передаёт направление движения и расстояние в этом направлении
    robotyck = (digitalRead(18) < 1);
  }
  
  KT1=current_position[axis];
  RoboSearch(axis,rdir*(-20.0)); //этот шаг небольшая хитрость. Щуп отводится от стенки на 30 мм
  robotyck = (digitalRead(18) < 1);

  while (robotyck){
  RoboSearch(axis,rdir*(-0.01));//чтобы не перегружался буфер. это вряд ли понадобится, но на всякий сделаем
  robotyck = (digitalRead(18) < 1);
  }

  KT2=current_position[axis]; 
  Wdist=abs(KT1-KT2);
...
inline void gcode_G8() {
  int rdir=1;
  float Rdist;
  Rdist=Sdistance(X_AXIS,rdir);
  RoboSearch(X_AXIS,Rdist-5.0);
  TycK(X_AXIS, Rdist, 3);
  //RoboSearch(X_AXIS,50.0);
  //RoboSearch(X_AXIS,-100.0);
  //float rshift=Rdist/(3+1);
  //Redge(X_AXIS, 15.0);
  //Redge(X_AXIS, 15.0);
  //Redge(X_AXIS, 15.0);
/*RoboSearch(Y_AXIS,-50.0);

//RoboSearch(X_AXIS,25.0);
bool endstopZ = (digitalRead(18) < 1);
float roboZ=0;
float Rk1, Rk2;
float RoboD, Dp;
while (endstopZ){
  roboZ = roboZ + 0.01;
  RoboSearch(Y_AXIS,roboZ);
  endstopZ = (digitalRead(18) < 1);
}
Rk1=current_position[Y_AXIS];
//RoboSearch(Z_AXIS,0.0);
...
Rk2=current_position[Y_AXIS];
Dp=abs((Rk1-Rk2)/4);
RoboD=current_position[Y_AXIS]+Dp;
RoboSearch(Y_AXIS,RoboD);
border();
RoboD=current_position[Y_AXIS]+Dp;
RoboSearch(Y_AXIS,RoboD);
border();
...
/*RoboSearch(X_AXIS,75.0);
 endstopZ = (digitalRead(18) < 1);
 roboZ=0;
while (endstopZ){
  roboZ = roboZ - 0.01;
  RoboSearch(Z_AXIS,roboZ);
  endstopZ = (digitalRead(18) < 1);
}
RoboSearch(Z_AXIS,0.0);
 ...
 *
 */
 // Миша. 23.11.20 Рабочая (не костыльная G28!)
inline void gcode_G28(const bool always_home_all) {
    axisRoboMove(1000.0, 1000.0, 1000.0, -1000.0); // паркуем X
    if (digitalRead(3)==HIGH) {
      set_axis_is_at_home(X_AXIS);
      axisRoboMove(-1000.0, 1000.0, -1000.0, -1000.0); // паркуем Y по кареткам C - D
    }
    if (digitalRead(2)==HIGH) {
      axisRoboMove(1000.0, -1000.0, 0.0, 0.0); // паркуем Y по кареткам A - B
        }
    set_axis_is_at_home(Y_AXIS);
    if (digitalRead(15) == HIGH) {
      axisRoboMove(-1000.0, -1000.0, -1000.0, 1000.0); // гоним каретки по X влево (в сторону X_MIN, Y_MIN)
      }
    if (digitalRead(14) == HIGH){
      axisRoboMove(1000.0, 1000.0, 1000.0, -1000.0); // каретки доехали до конца. Нивелируем люфт. Гоним каретки обратно
      }
    if (digitalRead(3) == HIGH && digitalRead(2) == HIGH){
      axisRoboMove(-100.0, -100.0, -100.0, 100.0); // Откатываем каретки назад (вправо) на 100мм.
      }
    if (digitalRead(14) == HIGH || digitalRead(3) == HIGH){
      axisRoboMove(0.0, -1000.0, -1000.0, 0.0, 150.0, false); // Паркуем по Z по нижнему концевику.
      set_axis_is_at_home(Z_AXIS);
      }
    /*if (digitalRead(3)==HIGH&&digitalRead(2)==HIGH){
      axisRoboMove(-250.0, -250.0, -250.0, 250.0, 150.0, false); // сдвигаем на центр стола по X
        }
    RoboDelay(200, true); // Задержка, чтобы принтер успел доехать до центра стола. Нужна для задержки асинхронного сценария!
    if ((digitalRead(14)==HIGH) || (digitalRead(15)==HIGH)){
      while(digitalRead(18) == LOW){
          axisRoboMove(-0.01, 0.01, 0.01, 0.01, 150.0, false); // паркуем Z
  ...
  inline void gcode_G29() {
    ...
    if (!g29_in_progress) {
      ...
      if (!no_action) set_bed_leveling_enabled(false);
        ...
        if (DEPLOY_PROBE()) {                           //Родион 25.11.20
          set_bed_leveling_enabled(abl_should_enable);
          return;
        }
      #endif
      ...
      if (abl_probe_index == 0) {
        ...
        measured_z = current_position[Z_AXIS];
          ...
          mean += measured_z;
          eqnBVector[index] = measured_z;                 //Для Адама
...
inline void gcode_G92() {
  ...
  #endif
 float Z1= 130.0;
 // float X1 = ((457.20-planner.get_axis_position_mm(B_AXIS))/2.0),
  //      Z1 = 350.0-sqrt(sq(405.0)-sq(65.4)-sq((1034.0-(457.0-planner.get_axis_position_mm(B_AXIS)))/2.0-55.0-49.6));//Старая парковка
  ...
  if (IS_G92_0) LOOP_XYZE(i) {
    if (parser.seenval(axis_codes[i])) {
      ...
      if (!NEAR_ZERO(d)
          ...
          if (i == Z_AXIS) current_position[Z_AXIS] = Z1;
          //#if i == X_AXIS
            //current_position[X_AXIS] = X1;
          //#endif
         // if (i == Z_AXIS) current_position[Z_AXIS]=sqrt(sq(405.0)-sq(115.0-49.6)-sq(X2))-353.0;
          else current_position[i] = v;      // Without workspaces revert to Marlin 1.0 behavior 
...
//current_position[Z_AXIS] =;
//float xzy;//Родион
//xzy=planner.get_axis_position_mm(A_AXIS);

 //   Z_AXIS=0.0;
 //   float X2;
// planner.set_position_mm(cart.x, cart.y, cart.z, cart.e);  
 //   Y_AXIS=0.0; 
//
//    X2 = ((1033.0-55.0-55.0-planner.get_axis_position_mm(A_AXIS))/2);  //расстояние между центрами кареток после парковки 09.11.20 Родион
//    V[Z_AXIS]=sqrt(sq(405.0)-sq(115.0-49.6)-sq(X2))-350.0;//  //присвоение оси Z
SERIAL_PROTOCOLPAIR("Speed:", feedrate_mm_s);
...
void process_parsed_command() {
  ...
  switch (parser.command_letter) {
    case 'G': switch (parser.codenum) {
      ...
      #if ENABLED(UNREGISTERED_MOVE_SUPPORT)
        case 7: gcode_G7(); break;                                // G7: Колхоз Родион 09.11.20
      #endif

      case 8: gcode_G8(); break; //Поиск балки Родион 21.11.20
